...
const isSVGDefsElementNode = (node) => node.tagName === "defs";
...
async function cloneNode(node, context, isRoot = false, addWordToFontFamilies) {
  ...
  if (ownerDocument && ownerWindow && isElementNode(node) && (isHTMLElementNode(node) || isSVGElementNode(node))) {
    ...
    cloned2.__skipStyleCopying = context.currentParentNode?.__skipStyleCopying || isSVGDefsElementNode(node);
    ...
    const style = context.currentNodeStyle = cloned2.__skipStyleCopying ? /* @__PURE__ */ new Map() : copyCssStyles(node, cloned2, isRoot, context);
    if (isRoot)
